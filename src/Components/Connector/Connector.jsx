  ...
  const sendRequest = async (command) => {
    ...
    switch (command) {
      ...
      case "StopTransaction":
        ...
        metaData.ocmfSignedMeterValues = ocmfSignedMeterValues;
        metaData.ocmfPrivateKey = ocmfPrivateKey;
        metaData.startMeterValue = connectors[id].startMeterValue;
        metaData.startTimestamp = connectors[id].startTimestamp;
        metaData.stopTimestamp = new Date();
        break;
      case "MeterValues":
        const initialSoc =
          lastSessionSample?.initialSoc ??
          Math.floor(Math.random() * (22 - 3 + 1)) + 3;
        ...
        console.log(
          `Generated session sample for connector ${id}:`,
          sessionSample
        );

        // Use the last sample for display
        const sample = Array.isArray(sessionSample)
          ? sessionSample[sessionSample.length - 1]
          : sessionSample;
  ...
  useEffect(() => {
    if (!settings.inTransaction && autoMetering) {
      ...
      setTimeout(async () => {
        ...
        await sendRequest("StatusNotification");
    ...
    if (settings.inTransaction) {
      ...
      console.log(
        `Transaction started on connector ${id} with car: ${selectedCar}`
      );
    }
          ...
          {lastSessionSample && (
            ...
            >
              ...
              <Grid container spacing={1}>
                <Grid item xs={6} sm={4}>
                  <b>State of Charge:</b> {lastSessionSample.soc}%
                </Grid>
                <Grid item xs={6} sm={4}>
                  <b>Power:</b> {lastSessionSample.power} W
                </Grid>
                <Grid item xs={6} sm={4}>
                  <b>Current:</b> {lastSessionSample.current} A
                </Grid>
                <Grid item xs={6} sm={4}>
                  <b>Voltage:</b> {lastSessionSample.voltage} V
                </Grid>
                <Grid item xs={6} sm={4}>
                  <b>Energy:</b> {lastSessionSample.energy} Wh
                </Grid>
                <Grid item xs={6} sm={4}>
                  <b>Timestamp:</b>{" "}
                  {lastSessionSample.timestamp.toLocaleString()}
                </Grid>
              </Grid>
            </Box>
          )}
